...
public class Day2
{
    private static Lazy<List<List<Handful>>> parsedInput = new(ParseInput);

    public static string Part1()
    {
        int total = 0;

        for (int i = 0; i < parsedInput.Value.Count; i++)
        {
            if (parsedInput.Value[i].All(handful =>
    ...
    public static string Part2()
    {
        ...
        foreach (List<Handful> game in parsedInput.Value)
        {
            (int red, int green, int blue) = (0, 0, 0);
            foreach (Handful handful in game)
            {
                red = Math.Max(red, handful.red);
                green = Math.Max(green, handful.green);
                blue = Math.Max(blue, handful.blue);
            }
    ...
    private static List<List<Handful>> ParseInput()
    {
        List<List<Handful>> games = new();
        foreach (string line in input.Split("\n"))
        {
            List<Handful> handfuls = new();
            foreach (string inputHandful in line.Split(": ")[1].Split("; "))
            {
                (int red, int green, int blue) = (0, 0, 0);
                foreach (string colourCount in inputHandful.Split(", "))
                {
                    string[] splitColourCount = colourCount.Split(" ");
                    int count = int.Parse(splitColourCount[0]);
                    switch (splitColourCount[1])
                    {
                        case "red":
                            red = count;
                            break;
                        ...
                        case "blue":
                            ...
                            break;
                        default:
                            throw new Exception($"Unexpected colour - {splitColourCount[1]}");
                    }
                }
                handfuls.Add(new Handful(red, green, blue));
            }
            games.Add(handfuls);
        }
        return games;
    ...
    private const string input = @"Game 1: 13 green, 3 red; 4 red, 9 green, 4 blue; 9 green, 10 red, 2 blue
